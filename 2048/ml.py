import torch
import torch.nn as nn
import torch.nn.functional as F
import random
from collections import deque
from utilities import *
from generate_lookup_table import generate_lookup_tables_dict

layer_size = 128
epsilon = 1.0
batch_size = 64
min_buffer = 2000
buffer = deque(maxlen=100000)


def store(state, action, reward, new_state, done, buffer):
    buffer.append((state, action, reward, new_state, done))

def sample(batch_size, buffer):
    batch = random.sample(buffer, batch_size)
    states, actions, rewards, next_states, dones = zip(*batch)
    return states, actions, rewards, next_states, dones


def select_action(state, epsilon, lut, qnet):
    can_left = lut["can_left"]
    can_right = lut["can_right"]
    valid = check_valid(combine_state(state), can_left, can_right)
    if(random.random() < epsilon):
        return random.choice(valid)
    
    with torch.no_grad():
        s = torch.tensor(state, dtype=torch.float32).unsqueeze(0)  # (1,16)
        q = qnet(s).squeeze(0)  # (4,)
        mask = torch.full((4,), float("-inf"))
        mask[valid] = 0.0
        q_masked = q + mask
        return int(torch.argmax(q_masked).item())


def training_step(batch, qnet, target_net, optimizer, gamma=0.99):
    states, actions, rewards, next_states, dones = batch
    states      = torch.tensor(states, dtype=torch.float32)      # (B,16)
    next_states = torch.tensor(next_states, dtype=torch.float32) # (B,16)
    actions     = torch.tensor(actions, dtype=torch.int64)        # (B,)
    rewards     = torch.tensor(rewards, dtype=torch.float32)     # (B,)
    dones       = torch.tensor(dones, dtype=torch.float32)       # (B,)
    # Predict with the training network
    q_vals = qnet(states)                          # (B,4)
    q_sa   = q_vals.gather(1, actions.unsqueeze(1)).squeeze(1)  # (B,)
    # Predict with the target network
    with torch.no_grad():
        max_next_q = target_net(next_states).max(1)[0] # We know the next state and we use this information to update the net
        target = rewards + gamma * max_next_q * (1-dones)

    loss = F.mse_loss(q_sa, target)

    optimizer.zero_grad()
    loss.backward()
    optimizer.step()


def train_episodes(qnet, target_net, optimizer, lut, buffer, epsilon, num_episodes):
    """
    Train the Q-network for a specified number of episodes.
    
    Parameters:
        qnet: Main Q-network
        target_net: Target Q-network
        optimizer: Adam optimizer
        lut: Lookup tables dictionary
        buffer: Replay buffer (deque)
        epsilon: Exploration rate
        num_episodes: Number of episodes to train
        
    Returns:
        Updated epsilon value
    """
    for episode in range(num_episodes):
        state = split_state(add_random_tile_state(0))
        done = False
        total_reward = 0

        while not done:
            # Choose action
            action = select_action(state, epsilon, lut, qnet)
            # Step environment
            next_state, reward, done = take_step(state, action, lut)
            # Store in buffer
            store(state, action, reward, next_state, done, buffer)
            # Update state
            state = next_state
            total_reward += reward
            # Start learning
            if len(buffer) > min_buffer:
                batch = sample(batch_size, buffer)
                training_step(batch, qnet, target_net, optimizer)

        # Decay exploration
        epsilon = max(0.05, epsilon * 0.995)

        print(f"Episode #: {episode}  Total Reward: {total_reward}  Buffer Size: {len(buffer)}")
    
    return epsilon


def load_checkpoint(checkpoint_path, qnet, target_net, optimizer):
    """
    Load a saved checkpoint into the networks and optimizer.
    
    Parameters:
        checkpoint_path: Path to the .pth checkpoint file
        qnet: Main Q-network
        target_net: Target Q-network
        optimizer: Adam optimizer
        
    Returns:
        Loaded epsilon value
    """
    checkpoint = torch.load(checkpoint_path)
    qnet.load_state_dict(checkpoint['qnet_state'])
    target_net.load_state_dict(checkpoint['target_net_state'])
    optimizer.load_state_dict(checkpoint['optimizer_state'])
    epsilon = checkpoint['epsilon']
    print(f"Checkpoint loaded from {checkpoint_path}")
    print(f"Resumed with epsilon: {epsilon:.4f}")
    return epsilon


def save_checkpoint(checkpoint_path, qnet, target_net, optimizer, epsilon):
    """
    Save the current model state to a checkpoint file.
    
    Parameters:
        checkpoint_path: Path to save the .pth file
        qnet: Main Q-network
        target_net: Target Q-network
        optimizer: Adam optimizer
        epsilon: Current exploration rate
    """
    torch.save({
        'qnet_state': qnet.state_dict(),
        'target_net_state': target_net.state_dict(),
        'optimizer_state': optimizer.state_dict(),
        'epsilon': epsilon
    }, checkpoint_path)
    print(f"Checkpoint saved to {checkpoint_path}")



if __name__ == "__main__":

    lut = generate_lookup_tables_dict()

    # Prompt user for checkpoint file
    checkpoint_path = input("Enter path to checkpoint file (or press Enter to start from scratch): ").strip()
    
    # Initialize networks
    qnet = nn.Sequential(
        nn.Linear(16, layer_size),
        nn.ReLU(),
        nn.Linear(layer_size, layer_size),
        nn.ReLU(),
        nn.Linear(layer_size, 4)
    )

    target_net = nn.Sequential(
        nn.Linear(16, layer_size),
        nn.ReLU(),
        nn.Linear(layer_size, layer_size),
        nn.ReLU(),
        nn.Linear(layer_size, 4)
    )

    target_net.load_state_dict(qnet.state_dict())
    optimizer = torch.optim.Adam(qnet.parameters(), lr=1e-3)
    
    # Load checkpoint if provided
    current_epsilon = epsilon
    if checkpoint_path:
        try:
            current_epsilon = load_checkpoint(checkpoint_path, qnet, target_net, optimizer)
        except FileNotFoundError:
            print(f"File {checkpoint_path} not found. Starting from scratch.")
        except Exception as e:
            print(f"Error loading checkpoint: {e}. Starting from scratch.")
    else:
        print("Starting training from scratch.")
    
    # Train for 5000 episodes
    current_epsilon = train_episodes(qnet, target_net, optimizer, lut, buffer, current_epsilon, 5000)
    
    # Save the checkpoint
    save_path = "new"+checkpoint_path if checkpoint_path else 'model_checkpoint.pth'
    save_checkpoint(save_path, qnet, target_net, optimizer, current_epsilon)
